...
const ParticipantPage = ({ company }: { company: any }) => {
	const structuredData: WithContext<Organization> = {
		...
		url: `https://www.oss4.ai/participant/${company.slug}`,
	...
	return (
		<>
			<NextSeo
				title={`${company.name} - OSS4AI participant`}
				description={company.description}
				openGraph={{
					title: `${company.name} - OSS4AI participant`,
					description: company.description,
			...
			<BreadcrumbJsonLd
				itemListElements={[
					...
					},
					{
						position: 3,
						name: company.name,
						item: `https://www.oss4.ai/participant/${company.slug}`,
					},
				]}
			/>
			<JsonLd<WithContext<Organization>> item={structuredData} />
			<Head>
				<link rel='canonical' href={`https://www.oss4.ai/participant/${company.slug}`} />
			</Head>
			{/* Google Analytics 4 */}
